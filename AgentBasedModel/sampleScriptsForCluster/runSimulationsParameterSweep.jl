...
idx = parse(Int, ENV["SLURM_ARRAY_TASK_ID"])
chunk_size = ceil(Int, length(indices)/10) # our cluster allows max 300 jobs at a time, so we always split into roughly 300 "chunks"
indices = indices[(idx-1)*chunk_size+1 : min(idx*chunk_size, end)]
